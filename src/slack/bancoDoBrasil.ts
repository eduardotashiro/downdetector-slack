...
/*-*-*-*-*-*-*-* INICIO BB *-*-*-*-*-*-*-*/
export async function tratarBB(services: any) {
    const dados = services.dados;
    const status = dados.status;
    const service = dados?.company

    /*-*-*-*-*-*-*-* WARNING *-*-*-*-*-*-*-*/
    if ((status === ServiceStatus.WARNING) && !BBIncidente) {
        BBIncidente = {
            inicio: Date.now(),
            nivel: status,
            alertaEnviado: false
    ...
    if (status === ServiceStatus.WARNING && BBIncidente && !BBIncidente.alertaEnviado) {
        BBIncidente.nivel = ServiceStatus.WARNING
    ...
    /*-*-*-*-*-*-*-* WARNING *-*-*-*-*-*-*-*/
    if (status === ServiceStatus.WARNING && BBIncidente?.nivel === ServiceStatus.DANGER) {
    ...
    /*-*-*-*-*-*-*-* DANGER *-*-*-*-*-*-*-*/
    if (status === ServiceStatus.DANGER && !BBIncidente) {
        BBIncidente = {
            inicio: Date.now(),
            nivel: status,
            alertaEnviado: false
    ...
    /*-*-*-*-*-*-*-* WARNING -> DANGER *-*-*-*-*-*-*-*/
    if (status === ServiceStatus.DANGER && BBIncidente && BBIncidente.nivel === ServiceStatus.WARNING) {
        BBIncidente.nivel = ServiceStatus.DANGER;
    ...
    if (status === ServiceStatus.SUCCESS && BBIncidente && BBIncidente.nivel === ServiceStatus.WARNING && !BBIncidente.alertaEnviado) {
        BBIncidente.nivel = ServiceStatus.SUCCESS
    ...
    if (status === ServiceStatus.SUCCESS && BBIncidente && BBIncidente.alertaEnviado) {
    ...
    if ((status === ServiceStatus.WARNING || status === ServiceStatus.DANGER) && BBIncidente) {
